...
- PC 悬停延迟 200ms，允许移入浮层操作；默认触屏点击、键盘聚焦使用同一预览。备料等启用 `quickOnTouch` 的入口由触屏轻点执行快捷操作、长按 500ms 查看预览；移动手指时取消长按，长按后的点击不得重复执行操作。沿用原生 `popover=auto`、单个打开、Esc 关闭、视口避让和长内容内部滚动，不改成详情抽屉。
