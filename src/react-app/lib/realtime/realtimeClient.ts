...
import { requestRealtimeBootstrapToken } from '@app/lib/realtime/realtimeAuth';
import type {
  RealtimeChannel,
  RealtimeServerEvent,
...
export class RealtimeClient {
  ...
  private transportOpening = false;
  ...
  async connect() {
    ...
    this.transportOpening = true;
    let realtimeToken: string | undefined;
    try {
      realtimeToken = await requestRealtimeBootstrapToken();
    } catch (error) {
      this.transportOpening = false;
      this.updateStatus('offline', {
        lastError: error instanceof Error ? error.message : '实时认证失败',
      });
    ...
    const socket = new WebSocket(resolveRealtimeUrl(channels, realtimeToken));
    this.socket = socket;
    this.transportOpening = false;
