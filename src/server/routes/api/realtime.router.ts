...
const REALTIME_BOOTSTRAP_TTL_SECONDS = 60;
const fallbackRequireUser = requireUser();

function realtimeBootstrapKey(token: string) {
  return `realtime:bootstrap:v1:${token}`;
}
...
const requireRealtimeUser = (async (c, next) => {
  const token = c.req.query('rt')?.trim();
  if (!token) return fallbackRequireUser(c, next);
  if (!/^[A-Za-z0-9_-]{32,128}$/.test(token)) {
    return c.json({ success: false, error: '实时认证凭据无效' }, 401);
  }

  let raw: string | null;
  try {
    raw = await redis.get(realtimeBootstrapKey(token));
    if (raw) await redis.del(realtimeBootstrapKey(token));
  } catch (error) {
    console.error('[realtime] bootstrap auth unavailable', error);
    return c.json({ success: false, error: '实时认证服务暂不可用' }, 503);
  }
  if (!raw) return c.json({ success: false, error: '实时认证凭据已失效' }, 401);

  try {
    const user = JSON.parse(raw) as AuthUser;
    if (!user?.id || !user.email || !user.name) throw new Error('invalid user');
    c.set('user', user);
  } catch {
    return c.json({ success: false, error: '实时认证凭据无效' }, 401);
  }
...
router.post('/token', requireUser(), async (c) => {
  ...
  await redis.set(
    realtimeBootstrapKey(token),
    JSON.stringify(user),
    'EX',
    REALTIME_BOOTSTRAP_TTL_SECONDS,
  );
  return c.json({
    success: true,
    data: { token, expiresInSeconds: REALTIME_BOOTSTRAP_TTL_SECONDS },
  });
});
