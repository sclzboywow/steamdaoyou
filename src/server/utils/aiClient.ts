...
import {
  assertOfficialGeneratedContentSafe,
  collectModerationText,
  shouldModerateGeneratedContent,
...
export async function generateAiText(options: AiTextOptions) {
  ...
  try {
    ...
    if (shouldModerateGeneratedContent()) {
      await assertOfficialGeneratedContentSafe({
        source: `ai:${options.sceneId}`,
        content: result.text,
      });
    }
    return result;
...
export async function generateAiObject<GENERATED, RESULT = GENERATED>(
  ...
  const request = startAiRequest(options);
  try {
    const result = await generateAiObjectInternal({
      ...options,
      abortSignal: request.signal,
    });
    if (shouldModerateGeneratedContent()) {
      await assertOfficialGeneratedContentSafe({
        source: `ai:${options.sceneId}`,
        content: collectModerationText(result.output),
      });
    }
    return result;
  } finally {
...
export async function generateAiArray<ELEMENT, RESULT = ELEMENT[]>(
  ...
  const request = startAiRequest(options);
  try {
    const result = await generateAiArrayInternal({
      ...options,
      abortSignal: request.signal,
    });
    if (shouldModerateGeneratedContent()) {
